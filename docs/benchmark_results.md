...
| `default_sr_retry` | Package default: original first, then x2/x3/x4, bicubic/Lanczos, plus CLAHE and sharpen variants. Stops on first valid decode. |
| `x2_nearest_only` | Cheap fallback: original first, then a single x2 nearest-neighbor variant. No CLAHE / sharpen. |

All configs used `formats="qr"` and `return_errors=True`.
...
2. Escalate to `x2_nearest_only` or `default_sr_retry` only for offline review,
